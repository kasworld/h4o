...
import (
	...
	"github.com/kasworld/h4o/util/stats"
)

// App contains the application state
type App struct {
	*appbase.AppBase                        // Embedded standard application object
	currentDemo      DemoI                  // Current demo
	dirData          string                 // Full path of the data directory
	scene            *node.Node             // Scene rendered
	demoScene        *node.Node             // Scene populated by individual demos
	...
	orbit  *orbit.OrbitControl // Orbit control
}

// DemoI is the interface that must be satisfied by all demos.
...
func Create() *App {
	...
	a := new(App)
	a.AppBase = appbase.New("h4o example", 800, 600)
	...
	// Create scenes
	a.demoScene = node.NewNode() // demoScene will be cleared before a new demo is started
	a.scene = node.NewNode()
	...
	a.orbit = orbit.NewOrbitControl(a.camera)
	...
	// Create frame rater
	a.frameRater = framerater.NewFrameRater(*oTargetFPS)
	...
	log.Info("Using data directory:%s", a.dirData)
	...
	a.Subscribe(eventtype.OnWindowSize, func(evname eventtype.EventType, ev interface{}) { a.OnWindowResize() })
	a.OnWindowResize()

	// Subscribe to key events
	a.Subscribe(eventtype.OnKeyDown, func(evname eventtype.EventType, ev interface{}) {
		kev := ev.(*appwindow.KeyEvent)
		if kev.Key == appwindow.KeyEscape { // ESC terminates the program
			a.Exit()
		} else if kev.Key == appwindow.KeyF11 { // F11 toggles full screen
			//a.Window().SetFullScreen(!a.Window().FullScreen()) // TODO
		} else if kev.Key == appwindow.KeyS && kev.Mods == appwindow.ModAlt { // Ctr-S prints statistics in the console
			a.logStats()
		}
	})
...
func (a *App) buildGui(demoMap map[string]DemoI) {
	...
	a.treeTests.Subscribe(eventtype.OnChange, func(evname eventtype.EventType, ev interface{}) {
		...
		if ok {
			a.setupScene()
			test := label.GetNode().UserData().(DemoI)
			test.Start(a)
			a.currentDemo = test
		}
	})
...
func (a *App) setupScene() {
	...
	a.SetCursor(appwindow.ArrowCursor)
	...
	cb.Subscribe(eventtype.OnChange, func(evname eventtype.EventType, ev interface{}) {
		if cb.Value() {
			a.camera.SetProjection(camera.Perspective)
		} else {
	...
	s1.Subscribe(eventtype.OnChange, func(evname eventtype.EventType, ev interface{}) {
...
func (a *App) Orbit() *orbit.OrbitControl {
...
func (a *App) Run() {
	...
	if *oCpuProfile != "" {
		...
		log.Info("Started writing CPU profile to: %s", *oCpuProfile)
	}
	...
	if *oExecTrace != "" {
		...
		defer trace.Stop()
		log.Info("Started writing execution trace to: %s", *oExecTrace)
	}

	a.AppBase.Run(a.Update)
}
...
func (a *App) Update(rend *renderer.Renderer, deltaTime time.Duration) {
	...
	if *oRenderStats {
		...
		if stats.Panels > 0 {
			log.Debug("render stats:%+v", stats)
		}
	}
