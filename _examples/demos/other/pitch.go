...
func (t *Pitch) onKey(evname eventtype.EventType, ev interface{}) { // TODO use keystate?
	...
	// Pitch up
	if kev.Key == appwindow.KeyW {
		q.SetFromAxisAngle(&yaxis, -step)
		t.base.QuaternionMult(&q)
	}
	// Pitch down
	if kev.Key == appwindow.KeyS {
		q.SetFromAxisAngle(&yaxis, step)
		t.base.QuaternionMult(&q)
	}
	// Heading left
	if kev.Key == appwindow.KeyA {
		q.SetFromAxisAngle(&zaxis, step)
		t.base.QuaternionMult(&q)
	}
	// Heading right
	if kev.Key == appwindow.KeyD {
		q.SetFromAxisAngle(&zaxis, -step)
		t.base.QuaternionMult(&q)
	}
	// Bank left
	if kev.Key == appwindow.KeyZ {
		q.SetFromAxisAngle(&xaxis, -step)
		t.base.QuaternionMult(&q)
	}
	// Bank right
	if kev.Key == appwindow.KeyX {
		q.SetFromAxisAngle(&xaxis, step)
		t.base.QuaternionMult(&q)
	}
	// Reset
	if kev.Key == appwindow.KeyR {
