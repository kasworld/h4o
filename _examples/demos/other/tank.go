...
func (t *TankTest) Start(a *app.App) {
	...
	t.a = a
	a.Orbit().SetEnabled(^orbit.OrbitKeys)
	...
	a.Subscribe(eventtype.OnKeyUp, t.onKey)
}
...
// Process key events
func (t *TankTest) onKey(evname eventtype.EventType, ev interface{}) {

	var state bool
	if evname == eventtype.OnKeyDown {
		state = true
	} else {
		state = false
	}
...
type TankModel struct {
	node       *node.Node // node with all tank meshes
