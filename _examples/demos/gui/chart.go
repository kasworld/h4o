...
func (t *GuiChart) Start(a *app.App) {
	...
	cbG1.Subscribe(eventtype.OnChange, func(name eventtype.EventType, ev interface{}) {
		if cbG1.Value() {
	...
	cbG2.Subscribe(eventtype.OnChange, func(name eventtype.EventType, ev interface{}) {
		if cbG2.Value() {
	...
	cbTitle.Subscribe(eventtype.OnChange, func(name eventtype.EventType, ev interface{}) {
		if cbTitle.Value() {
	...
	cbX.Subscribe(eventtype.OnChange, func(name eventtype.EventType, ev interface{}) {
		if cbX.Value() {
	...
	cbY.Subscribe(eventtype.OnChange, func(name eventtype.EventType, ev interface{}) {
		if cbY.Value() {
	...
	rStartx.SetValue(0)
	rStartx.Subscribe(eventtype.OnChange, func(evname eventtype.EventType, ev interface{}) {
	...
	rStepx.Subscribe(eventtype.OnChange, func(evname eventtype.EventType, ev interface{}) {
	...
	rMiny.SetValue(minY)
	rMiny.Subscribe(eventtype.OnChange, func(evname eventtype.EventType, ev interface{}) {
		minY = rMiny.Value()
		chart.SetRangeY(minY, maxY)
	})
	...
	rMaxy.SetValue(maxY)
	rMaxy.Subscribe(eventtype.OnChange, func(evname eventtype.EventType, ev interface{}) {
		maxY = rMaxy.Value()
		chart.SetRangeY(minY, maxY)
	})
	...
	cbAutoy.Subscribe(eventtype.OnChange, func(evname eventtype.EventType, ev interface{}) {
		if cbAutoy.Value() {
	...
	a.DemoPanel().Subscribe(eventtype.OnResize, func(evname eventtype.EventType, ev interface{}) {
		chart.SetSize(a.DemoPanel().ContentWidth(), a.DemoPanel().ContentHeight()-chartY)
	})
}
...
func newRanger(width, height, min, max float32, format string) *ranger {
	...
	r.Slider.Subscribe(eventtype.OnChange, func(evname eventtype.EventType, ev interface{}) {
