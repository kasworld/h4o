...
func (t *GuiTabBar) Start(a *app.App) {
	...
	a.DemoPanel().Dispatch(eventtype.OnResize, nil)
}
...
func (t *GuiTabBar) setTabMenu(a *app.App, tab *gui.Tab) {
	...
	menu.Subscribe(eventtype.OnClick, func(evname eventtype.EventType, ev interface{}) {
		menu.SetVisible(false)
	...
	menu.Subscribe(eventtype.OnMouseDownOut, func(evname eventtype.EventType, ev interface{}) {
		menu.SetVisible(false)
	})
	...
	header := tab.Header()
	header.Add(menu)
	header.Subscribe(eventtype.OnMouseDown, func(evname eventtype.EventType, ev interface{}) {
		if ev.(*appwindow.MouseEvent).Button == appwindow.MouseButtonRight {
