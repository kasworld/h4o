...
func (t *GuiTable) Start(a *app.App) {
	...
	// Resize table
	a.DemoPanel().SubscribeID(eventtype.OnResize, a, func(evname eventtype.EventType, ev interface{}) {
		tab.SetSize(a.DemoPanel().ContentWidth(), a.DemoPanel().ContentHeight()-tableY)
	})
	...
	mCol.Subscribe(eventtype.OnMouseDownOut, func(evname eventtype.EventType, ev interface{}) {
		mCol.SetVisible(false)
	})
	tab.Add(mCol)
	...
	mRow.Subscribe(eventtype.OnMouseDownOut, func(evname eventtype.EventType, ev interface{}) {
		mRow.SetVisible(false)
	})
	tab.Add(mRow)
	...
	tab.Subscribe(eventtype.OnTableClick, func(evname eventtype.EventType, ev interface{}) {
		tce = ev.(gui.TableClickEvent)
		//log.Debug("evname:%v -> %+v", evname, e)
		if tce.Button != appwindow.MouseButtonRight {
			return
		}
		if tce.Header {
	...
	updateStatus := func(evname eventtype.EventType, ev interface{}) {
	...
	tab.Subscribe(eventtype.OnTableRowCount, updateStatus)
	tab.Subscribe(eventtype.OnChange, updateStatus)

	// Subscribe to column menu
	mCol.Subscribe(eventtype.OnClick, func(evname eventtype.EventType, ev interface{}) {
		mCol.SetVisible(false)
	...
	mRow.Subscribe(eventtype.OnClick, func(evname eventtype.EventType, ev interface{}) {
		mRow.SetVisible(false)
		opid := ev.(*gui.MenuItem).Id()
		switch opid {
		case "delRow":
	...
	mb.Subscribe(eventtype.OnClick, func(evname eventtype.EventType, ev interface{}) {
		opid := ev.(*gui.MenuItem).Id()
		switch opid {
		case "showHeader":
			tab.ShowHeader(true)
