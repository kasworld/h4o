...
func NewFileSelectButton(path, text string, width, height float32) *FileSelectButton {
	...
	fsb.Button.Subscribe(eventtype.OnClick, func(evname eventtype.EventType, ev interface{}) {
...
func NewFileSelect(width, height float32) *FileSelect {
	...
	fs.list.Subscribe(eventtype.OnChange, func(evname eventtype.EventType, ev interface{}) {
		fs.onSelect()
	})
	fs.Add(fs.list)
